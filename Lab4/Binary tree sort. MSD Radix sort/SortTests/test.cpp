...
#include <random>
#include <cstdint>
#include "D:\Óíèê\ÏÏÎÈÑ\4LABA\task1\task1\sort.h"
using namespace sortlib; 

// Âñïîìîãàòåëüíûé êëàññ äëÿ ïðîâåðêè ñîðòèðîâêè ïîëüçîâàòåëüñêèõ òèïîâ
struct Student {
    std::string name;
    int score{};
...
// Óíèâåðñàëüíàÿ ïðîâåðêà ñîâïàäåíèÿ ñî std::sort
...
static void expect_sorted_equal(Container c1, Compare comp = {}) {
    ...
    // Âûçûâàåì íàøó ñîðòèðîâêó è ñâåðÿåìñÿ ñî ñòàíäàðòíîé
...
TEST(BinaryTreeSort, CustomObjectsVector) {
    ...
    // Îæèäàåì ñîâïàäåíèÿ ñî std::sort
...
// -------- Ñðàâíåíèå àëãîðèòìîâ íà îäíîì íàáîðå ---------
...
// Äîïîëíèòåëüíûå ãðàíè÷íûå ñëó÷àè
...
// Äîïîëíèòåëüíûå òåñòû äëÿ ïîâûøåíèÿ ïîêðûòèÿ äî 85%+
...
// Äîïîëíèòåëüíûå òåñòû äëÿ ïîêðûòèÿ âñåõ ÿâíûõ èíñòàíöèðîâàíèé
...
// Òåñòû äëÿ ïîêðûòèÿ ìåòîäîâ BinarySearchTree
...
// Äîïîëíèòåëüíûå òåñòû äëÿ óâåëè÷åíèÿ ïîêðûòèÿ ðåêóðñèâíûõ ïóòåé
...
// Äîïîëíèòåëüíûå òåñòû äëÿ ïîêðûòèÿ âñåõ âåòîê ðåêóðñèè

TEST(MSDRadixSort, MultiByteRecursion) {
    std::vector<int> v;
    // Ñîçäàåì ÷èñëà ñ ðàçíûìè áàéòàìè äëÿ ãëóáîêîé ðåêóðñèè
    for (int i = 0; i < 256; ++i) {
        v.push_back(i << 24);
        v.push_back(i << 16);
        v.push_back(i << 8);
